...
class Resuscitation {
  ...
  enableResuscitation(pattern) {
    switch (pattern) {
      case "continuous":
        this.continue_resuscitation = true;
        break;
      case "3:1":
        this.continue_resuscitation = false;
        this.ratio_comp = 3;
        this.ratio_resp = 1;
        break;
      case "15:2":
  ...
  disableResuscitation() {
    this.is_enabled = false;
    this._model.components["LV"].pres_ext = 0;
    this._model.components["RV"].pres_ext = 0;
    this._model.components["LA"].pres_ext = 0;
    this._model.components["RA"].pres_ext = 0;
    this._model.components["AA"].pres_ext = 0;
    this._model.components["AARCH"].pres_ext = 0;
    this._model.components["VCII"].pres_ext = 0;
    this._model.components["APC"].pres_ext = 0;
    this._model.components["PV"].pres_ext = 0;
    this._model.components["LL"].pres_ext = 0;
    this._model.components["LR"].pres_ext = 0;

    this._model.components["CHEST_L"].pres_ext = 0;
    this._model.components["CHEST_R"].pres_ext = 0;
  }
  ...
  modelCycle() {
    ...
    this._sinus =
      a_sinus +
      b_sinus * Math.sin(c_sinus * this._comp_counter - 0.5 * Math.PI);
  ...
  continue() {
    ...
    this._model.components["CHEST_L"].pres_ext = 0.001 * this._sinus;
    this._model.components["CHEST_R"].pres_ext = 0.001 * this._sinus;
  ...
  discontinue() {
    ...
    // Apply respiratory support
    let total_time_vent =
      this._model.components["Ventilator"].t_in +
      this._model.components["Ventilator"].t_ex;

    if (
      this._resp_counter <= this.ratio_resp * total_time_vent &&
      this._comp_counter >= this.ratio_comp / this.f_comp
    ) {
      this._model.components["Ventilator"].is_enabled = true;
      // update counter
